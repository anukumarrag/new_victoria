 ...
 * @author anurag
 */
public class EmailDataforDOA {
    public static List<Guest> getEmails() throws SQLException {
        List emails=new ArrayList();
        Connection con=null;
        try{
            con=new Database().getConnection();
            ResultSet  rs=con.createStatement().executeQuery("SELECT * FROM guest WHERE doa!='' and MONTH(doa) = MONTH(CURRENT_DATE) AND day(doa) = DAY(CURRENT_DATE);");
            while(rs.next()){
                Guest g=new Guest();
